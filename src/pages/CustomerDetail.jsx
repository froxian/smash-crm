...
export default function CustomerDetail() {
  ...
  // Add states
  const [addMode, setAddMode] = useState(null) // 'note' | 'task'
  ...
  async function fetchAll() {
    ...
    const [cRes, nRes, tRes, uRes] = await Promise.all([
      supabase.from('customers').select('*, owner:users!owner_id(nickname)').eq('id', id).single(),
      supabase.from('notes').select('*, author:users!author_id(nickname)').eq('customer_id', id).order('created_at', { ascending: false }),
      supabase.from('tasks').select('*, creator:users!created_by(nickname), assignee:users!assigned_to(nickname)').eq('customer_id', id).order('created_at', { ascending: false }),
  ...
  async function addNote() {
    ...
    const { data } = await supabase.from('notes')
      .insert([{ customer_id: id, text: noteText, author_id: user.id }])
  ...
  if (!customer) return <div className="flex-1 flex items-center justify-center" style={{ color: 'var(--muted)', fontSize: 13 }}>Customer not found</div>

  // Timeline: notes + tasks mixed
  const items = [
      ...
      <div style={{ maxWidth: 500, margin: '28px auto', padding: '0 16px' }}>
        ...
        <div style={{ ...cardStyle, overflow: 'hidden' }}>
          <div className="text-center" style={{ padding: '30px 24px 4px' }}>
            <div className="font-bold" style={{ fontSize: 20 }}>{customer.name}</div>
            {customer.description && <div style={{ fontSize: 12, color: 'var(--sub)', marginTop: 5 }}>{customer.description}</div>}
            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 8, marginBottom: 20 }}>
              {customer.contact || 'No contact added'}
            </div>
          </div>
          ...
          <div style={{ borderTop: '1px solid var(--border)' }}>
            <div style={fieldRow}>
              ...
              <span style={{ fontSize: 13, fontWeight: 500 }}>{customer.owner?.nickname || '—'}</span>
            </div>
            <div style={fieldRow}>
              <span style={{ fontSize: 13, color: 'var(--sub)' }}>Contact</span>
              <span style={{ fontSize: 13, fontWeight: 500 }}>{customer.contact || '—'}</span>
            </div>
                ...
                {Object.entries(ST).map(([k, v]) => (
                  <button key={k} onClick={() => updateField('status', k)}
                    style={{
                      ...
                      border: customer.status === k ? `2px solid ${v.color}` : '1px solid var(--border)',
                      background: customer.status === k ? v.color + '12' : 'transparent',
                      color: customer.status === k ? v.color : 'var(--muted)',
                      transition: 'all 0.1s',
                    }}
                  >{v.label}</button>
                ))}
        ...
        <div className="flex gap-2.5" style={{ marginTop: 18 }}>
          <button
            ...
            style={{
              ...cardStyle, padding: '16px 12px', fontSize: 12,
              color: addMode === 'note' ? 'var(--green)' : 'var(--sub)',
              borderColor: addMode === 'note' ? 'var(--green)' : 'var(--border)',
          ...
          <button
            ...
            style={{
              ...cardStyle, padding: '16px 12px', fontSize: 12,
              color: addMode === 'task' ? 'var(--green)' : 'var(--sub)',
              borderColor: addMode === 'task' ? 'var(--green)' : 'var(--border)',
              transition: 'all 0.15s',
            }}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
          ...
          <div style={{ ...cardStyle, marginTop: 14, padding: '16px 18px' }}>
            <textarea
              ...
              style={{
                ...
                borderRadius: 8, padding: '10px 12px', background: 'var(--input-bg)',
                fontSize: 13, color: 'var(--text)', minHeight: 70,
              }}
            />
            <div className="flex justify-between items-center" style={{ marginTop: 10 }}>
              ...
              <div className="flex gap-2">
                <button onClick={() => setAddMode(null)}
                  style={{ padding: '6px 14px', borderRadius: 7, border: '1px solid var(--border)', background: 'transparent', color: 'var(--sub)', fontSize: 12 }}>
                  Cancel
                </button>
                <button onClick={addNote}
                  style={{ padding: '6px 14px', borderRadius: 7, border: 'none', background: 'var(--accent)', color: '#fff', fontSize: 12, fontWeight: 600, boxShadow: 'var(--shadow-sm)' }}>
                  Save note
                </button>
              </div>
            </div>
        ...
        {addMode === 'task' && (
          <div style={{ ...cardStyle, marginTop: 14, padding: '16px 18px' }}>
            <input
              ref={taskRef} value={taskText} onChange={e => setTaskText(e.target.value)}
              placeholder="Task description..."
              onKeyDown={e => { if (e.key === 'Enter') addTask(); if (e.key === 'Escape') setAddMode(null) }}
              style={{
                width: '100%', border: '1px solid var(--border)', borderRadius: 8,
                padding: '10px 12px', background: 'var(--input-bg)', fontSize: 13, color: 'var(--text)',
              }}
            />
        ...
        <div className="flex flex-col gap-2" style={{ marginTop: 22, marginBottom: 40 }}>
          {items.length === 0 && !addMode && (
            <div className="text-center" style={{ ...cardStyle, padding: 32, color: 'var(--muted)', fontSize: 13 }}>
